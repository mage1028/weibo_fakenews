...
class FakeNewsSpider(Spider):
    ...
    for i in range(600, 900):
        urls.append(
            ['https://service.account.weibo.com/?type=5&status=4&page={0}'.format(i)])
    ...
    def parse(self, response):
        ...
        for i in range(10, 30):
            url = 'https://service.account.weibo.com/' + urls[i]
    ...
    def parse_fake(self, response):
        ...
        html = re.findall(r'<script>[\s\S]*?</script>',
                          text)[0].strip('<script>').strip('</script>')

        html = html.strip(
            'STK && STK.pageletM && STK.pageletM.view(').strip(')')

        html = json.loads(html)['html']
        people = []
        item['reason'] = Selector(text=html).xpath(
            '//*[@class="p"]/text()').extract()
        item['reason'] = dealstr(''.join(item['reason']))
        contents = Selector(text=html).xpath('//*[@class="con"]').extract()
        for content in contents:

            try:
                people.append(re.findall(
                    r'blank\">[\s\S]*?</a>', content)[0].strip('blank\">').strip('</a>'))
                content = Selector(text=content).xpath(
                    '//*[@class="con"]/input/@value').extract()[0]
                ...
                try:
                    content = re.findall(
                        r'/a>[\s\S]*?</div>', content)[0].strip('/a>').strip('</div>')
                    content = dealstr(content)
                except:
                    content = '已被删除'
                informers_content.append(content)
        ...
        informants_time = Selector(text=html).xpath(
            '//div[@class="item top"]/p[@class="publisher"]/text()').extract()
        informants_time = ''.join(informants_time).split('发布时间：')[1].split()
        item['informants_time'] = ' '.join(informants_time)
        informants_url = Selector(text=html).xpath(
            '//*[@class="item top"]/div/a/@href').extract()[0]
        ...
        try:
            url = Selector(text=html).xpath(
                '//p[@class="publisher"]/a/@href').extract()[0]
            a = {
            ...
            yield Request(url=url, callback=self.parse_fakeweibo, meta={'item': item}, headers=a)
        except:
            item['content'] = backup
    ...
    def parse_fakeweibo(self, response):
        ...
        # 处理赞，转发 ，评论
        try:
            forwarded_count = re.findall(
                r'<em>[\s\S]*?em>', text)[4].lstrip('<em>').split('<')[0]
            if forwarded_count == '转发':
                forwarded_count = '0'
            item['forwarded_count'] = forwarded_count
        except:
            forwarded_count = re.findall(
                r'<em>[\s\S]*?em>', text)[1].lstrip('<em>').split('<')[0]
            if forwarded_count == '转发':
                forwarded_count = '0'
            item['forwarded_count'] = forwarded_count
        try:
            comment_count = re.findall(
                r'<em>[\s\S]*?em>', text)[5].lstrip('<em>').split('<')[0]
            if comment_count == '评论':
                comment_count = '0'
            item['comment_count'] = comment_count
        except:
            comment_count = re.findall(
                r'<em>[\s\S]*?em>', text)[2].lstrip('<em>').split('<')[0]
            if comment_count == '评论':
                comment_count = '0'
            item['comment_count'] = comment_count
        try:
            like_count = re.findall(
                r'<em>[\s\S]*?em>', text)[6].lstrip('<em>').split('<')[0]
            if like_count == '赞':
                like_count = '0'
            item['like_count'] = like_count
        except:
            like_count = re.findall(
                r'<em>[\s\S]*?em>', text)[3].lstrip('<em>').split('<')[0]
            if like_count == '赞':
                like_count = '0'
            item['like_count'] = like_count
        # 处理url
    ...
    def parse_comments(self, response):
        ...
        html = html.split('\\n')
        html = ''.join(html)
        author_comments = Selector(text=html).xpath(
            '//*[@class="WB_text"]').extract()

        for author_comment in author_comments:

            item['author'].append(
                Selector(text=author_comment).xpath('//a/text()').extract()[0])
            remove_author = Selector(text=author_comment).xpath(
                '//a/text()').extract()[0]
